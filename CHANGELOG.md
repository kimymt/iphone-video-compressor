...
## [1.2.4] - 2026-09-05 — 動画選択後に先へ進まない問題を修正

### Fixed

- 動画の推定時間を取得する補助処理が応答しない場合でも、動画をキューに追加して
  圧縮を開始できるように修正。
- 推定用 Worker のエラー、メッセージ解析エラー、送信失敗、10秒のタイムアウトを
  安全に終了させ、選択画面が待ち続ける状態を防止。
- 遅れて届いた推定情報が、圧縮で確定した時間を上書きしたり、削除済みの
  キュー項目を復活させたりしないよう修正。
- 失敗時の利用者向け表示と WebKit E2E テストの期待値を揃え、自動検証のずれを解消。
