...
34. **`<input type=file>.click()` も iOS Safari の transient user activation 制約下にある**。`await` を挟むと file picker が silent drop され、ユーザのタップに何も起きない（v1.2.3 で実機 iPhone から再現報告: 起動直後 + cancel 後の 2 回目タップで picker が開かない）。ハマりどころ #29 の Wake Lock と同根だが、`navigator.wakeLock.request()` は `acquire()` 内で同期 call すれば await の後でも届くのに対し、`input.click()` 自体が同期で発火する必要がある。修正: `handleClick` を非 async にして、`wakeLockManager.acquire()` → `inputRef.current.value = ''` → `inputRef.current.click()` → `void unlockAudio()` の順で同期発火（v1.2.3 PR、`src/components/FilePicker.tsx`）。加えて、前回値が残っていると iOS Safari は picker 再表示を skip する（同じファイル選択でも `change` が飛ばない既知の挙動と同根）ので、click の直前に必ず `value = ''` でリセットする
